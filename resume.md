...
Última actualización: 2026-07-26 (séptima sesión). Este archivo existe para que cualquier agente (o persona) pueda retomar el trabajo sin releer toda la conversación anterior.
...
- **004-solicitud-alta-taller** (sexta sesión): completo — backend (`solicitudes_taller`/`solicitudes_taller_historial`, modelos, 6 Actions de máquina de estados con `lockForUpdate`), UI pública (`/solicitudes-taller/nueva`, formulario multi-paso; `/solicitudes-taller/{token}`, seguimiento) y Filament (`SolicitudTallerResource` en `/admin`). 44 tests. Pendiente a propósito: detección de duplicados (criterio opcional del spec, no activado). **UI pública todavía en el layout mínimo pre-016 (`layouts/marketplace.blade.php`), no se migró al design system Awesomic en la séptima sesión — queda pendiente para una sesión de pulido.**
- **005-marketplace-busqueda-perfil + 016-ui-design-system** (séptima sesión): completos — tokens Awesomic + temas Filament v5 (`->viteTheme()`+`->font()`), Alpine.js instalado (no lo estaba), layouts/componentes Blade del marketplace, `GET /` (home), `GET /talleres/buscar`, `GET /talleres/{slug}` (perfil, 404 si no público), `GET /api/talleres/search` (JSON, `throttle:30,1`). Dos bugs reales corregidos en `HasGeolocation` (`scopeConDistanciaA` perdía columnas del modelo, `scopeCercanoA` no filtraba nada por usar `ST_DWithin` en grados en vez de metros — ver detalle arriba). 21 tests nuevos. Pendiente a propósito: dashboard de usuario marketplace y reseñas reales (dependen de `006`), verificación visual en navegador real.
- **Total: 172/172 tests Pest verdes**, `laravel/pint` sin pendientes.
...
- Git: repositorio en rama `specs/planificacion`. Todo el código hasta `004-solicitud-alta-taller` inclusive está commiteado y pusheado en `origin/specs/planificacion` (`eca323c` → `08c2d90` → `dee42c2` → `96bce0d` → `c42d59e`). **El código de `005`+`016` (séptima sesión) todavía NO está commiteado ni pusheado — pendiente.**
...
7. ~~Implementar `005-marketplace-busqueda-perfil` + `016-ui-design-system`~~ **Hecho** (séptima sesión, código todavía sin commitear — ver arriba).
8. Commitear y pushear el código de `005`+`016`. Luego seguir en orden de dependencia: `006-resenas-favoritos` → `007`…`015`. Al llegar a `006-resenas-favoritos`, recordar tres cosas pendientes de sesiones previas: (a) implementar el listener de recálculo de `calificacion_promedio`/`cantidad_resenas` de `Taller` sobre eventos de `Resena` (columnas y defaults ya listos desde `003`), (b) decidir cómo asignar el rol `MARKETPLACE_USER` (recomendación ya documentada: chequear permisos `marketplace.*` sin pasar por `asignaciones_rol`, ver brecha estructural de la tercera sesión más abajo), y (c) construir `<x-marketplace.review-card>`/`Alpine.data('starRating')` ya están listos desde `005`+`016` — reusarlos, no reconstruirlos.
9. ~~Al completar una feature con código + tests que cubran sus criterios de aceptación **y su UI**, actualizar `status: implemented`~~ **Hecho para 001, 002, 003, 004, 005 y 016** (005/016 marcados en la séptima sesión).
