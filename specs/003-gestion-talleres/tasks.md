...
- [ ] Listener de recálculo de `calificacion_promedio`/`cantidad_resenas` sobre eventos de `Resena` — **bloqueado**: el modelo `Resena` no existe todavía (`006-resenas-favoritos`). Columnas y defaults (0) ya están listos.
...
- [x] Advertencia al Super Admin antes de soft-deletear taller con entidades hijas activas: modal de confirmación con texto estático (sin conteo real de hijos — **bloqueado** hasta que existan modelos con `BelongsToTaller` en `007+`, el texto se actualizará ahí).
...
- [ ] Test de recálculo de calificación excluyendo reseñas no publicadas — bloqueado junto con el listener de arriba (depende de `Resena`, spec `006`).
